...
function Wishlist() {
  ...
  const fetchWishlist = async () => {
    try {
      ...
      const favoriteResponse = await fetch("/api2/favorite/get_all", {
        method: "GET",
        headers: {
      ...
      if (favoriteData.successful && Array.isArray(favoriteData.favorites)) {
        ...
        const listingResponse = await fetch("/api2/listing/get_all", {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const handleRemoveFromWishlist = async (id) => {
    try {
      const response = await fetch("/api2/favorite/remove", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
          ...
          {wishlist.length > 0 ? (
            <div className="row gx-4 gy-4">
              {wishlist.map((listing, index) => (
                <div key={listing.id || index} className="col-lg-4 col-md-6 d-flex align-items-stretch">
                  <div className="card h-100 w-100 shadow d-flex flex-column">
                    ...
                    <div className="card-body text-center d-flex flex-column justify-content-between">
                      ...
                      <p><strong>Starting Price:</strong> ${listing.minimum_bid}</p>
                      <p><strong>Current Bid:</strong> ${currentBids[listing.id] ?? "Loading..."}</p>
                      <p><strong>Buy-Now Price:</strong> ${listing.buy_now}</p>

                      <div className="mt-auto d-flex justify-content-between gap-2">
                        <button className="btn btn-danger flex-grow-1" onClick={() => handleRemoveFromWishlist(listing.id)}>
                          Remove
                        </button>
                        <button className="btn btn-primary flex-grow-1" onClick={() => handleShowDetails(listing)}>
                          View Details
                        </button>
                      </div>
                    </div>
...
{modalShow && selectedItem && (
  <div className="custom-modal">
    <div className="custom-modal-content">
      ...
      <Box
        component="img"
        src={selectedItem.image_urls?.[selectedImageIndex] || "/placeholder.jpg"}
      ...
      <p><strong>Starting Price:</strong> ${selectedItem.minimum_bid}</p>
      <p><strong>Current Bid:</strong> ${currentBids[selectedItem?.id] ?? "Loading..."}</p>
      <p><strong>Buy-Now Price:</strong> ${selectedItem.buy_now}</p>

      {/* Buttons Section */}
      <div className="d-flex justify-content-between mt-3 gap-2">
        <button className="btn btn-danger flex-grow-1" onClick={handleCloseModal}>Close</button>
        <Link to="/place-bid" state={{ ...selectedItem, current_bid: currentBids[selectedItem.id] || 0 }} className="btn btn-success flex-grow-1 text-center">Place Bid</Link>
        <button className="btn btn-warning flex-grow-1" onClick={() => navigate("/checkout", { state: { productName: selectedItem.title, price: selectedItem.buy_now } })}>Buy Now</button>
      </div>
    </div>
  </div>
)}
