...
export default function SellerOwnListing() {
    ...
    const fetchListings = async () => {
        try {
            const response = await fetch('/api2/listing/get_own', {
                method: "GET",
                headers: {
    ...
    const getDisplayedList = () => {
        if (selectedTab === 'upcoming') 
            return (
                ...
                <h1 className="mb-4 text-center text-uppercase">Upcoming Auction Listings</h1>
                <Section list={UpcomingList} type="upcoming" navigate={navigate} setListings={setListings} listings={listings}/>
                </div>
            );
            if (selectedTab === 'ongoing') 
            return (
                <div className="container my-5">
                <h1 className="mb-4 text-center text-uppercase">Ongoing Auction Listings</h1>
                <Section list={CurrentList} type="current" navigate={navigate} setListings={setListings} listings={listings}/>
                </div>
            );
            if (selectedTab === 'past') 
            return (
                <div className="container my-5">
                <h1 className="mb-4 text-center text-uppercase">Auction Listing History</h1>
                <Section list={PastList} type="past" navigate={navigate} setListings={setListings} listings={listings}/>
                </div>
            );
        };
    ...
    return (
        <Container id="SellerAuctionListings" sx={{ display: 'flex', flexDirection: 'row', pt: 2, m: 0 }}>
            <Box sx={{ width: '200px', flexShrink: 0, borderTop: '2px solid #ccc', borderRight: '2px solid #ccc', p: 2, minHeight: '100vh', backgroundColor: '#f8f8f8' }}>
                <Typography variant="h5" sx={{ fontWeight: 'bold', mb: 2 }}>Manage My Listings</Typography>
                <Button fullWidth sx={tabButtonStyle(selectedTab === 'upcoming')} onClick={() => setSelectedTab('upcoming')}>Upcoming Auction Listing</Button>
                <Button fullWidth sx={tabButtonStyle(selectedTab === 'ongoing')} onClick={() => setSelectedTab('ongoing')}>Ongoing Auction Listing</Button>
                <Button fullWidth sx={tabButtonStyle(selectedTab === 'past')} onClick={() => setSelectedTab('past')}>Auction Listing History</Button>
            </Box>
...
function Section({ title, list, type, navigate, setListings, listings}) {
    ...
    const handleConfirmDelete = async () => {
        ...
        const imageUrls = listings.find(item => item.id === listingId)?.image_urls; // Get all image URLs of the selected listing
        ...
        try {
            ...
            const deleteListingResponse = await fetch(`/api2/listing/delete/${listingId}`, {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
    ...
    return (
        <Container id="SAuctionListings" sx={{ display: 'flex', flexDirection: 'column',  m:0 }}>
            ...
            <Grid container spacing={4} sx={{ width: '100%', justifyContent: 'flex-start', m:0 }}>
                {list.map((item, index) => (
                    <Grid item xs={12} sm={4} md={4} key={index} sx={{ display: 'flex' }}>
                        <Card variant="outlined" sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', flexGrow: 1, width: '100%' }}>
                            <CardContent>
                                <Box
                                    component="img"
                                    src={Array.isArray(item.image_urls) && item.image_urls.length > 0 ? item.image_urls[0] : ""}
                                    alt={item.title}
                                    sx={{
                                        width: '100%',
                                        height: '300px',
                ...
                <Dialog open={open} onClose={() => setOpen(false)}>
                    <DialogTitle sx={{ fontSize: '2rem', fontWeight: 'bold' }}>Confirm Deletion</DialogTitle>
                    <DialogContent sx={{ fontSize: '1.5rem' }}>Are you sure you want to delete this listing?</DialogContent>
                    <DialogActions>
                        <Button sx={{ fontSize: '1.5rem' }} onClick={() => setOpen(false)}>Cancel</Button>
                        <Button sx={{ fontSize: '1.5rem' }} onClick={handleConfirmDelete} color="error">Delete</Button>
                    </DialogActions>
                </Dialog>
        </Container>
    );
...
function getSubheader(item, type) {
    if (type === 'upcoming') {
        return (
            <>
                <div>Auction Start Date:<br />{cleanDateString(item.start_at)}</div>
        ...
        return (
            <>
                <div>Auction ended on:<br />{cleanDateString(item.end_at)}</div>
            </>
        );
    }
}
