...
const ImageSlider = ({ images }) => {
        ...
        if (images.length === 1) {
            return <div className="image-slider"><img src={images[0]} alt="Image" /></div>;
        }
...
export default function CreateAuction() {
    ...
    const auctionDescriptions = {
        English: "*English auction starts at a low price and increases until it reaches the auction end date.",
        Dutch: "*Dutch auction starts at a high price and will decrease until it reaches the auction end date",
        "Sealed-Bid": "*Sealed-Bid auction, all bidders submit their bids in secret, and the highest bid wins.",
    };
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await fetch('/api2/listing/create', {
                method: 'POST',
                headers: {
            ...
            if (formData.image_urls.length > 0) { // UPDATED
    ...
    return (
        ...
        <form onSubmit={handleSubmit}>
            ...
            <h1 className="mb-4 text-center text-uppercase">Create Listing</h1></div>
                <Grid container spacing={3}>
                    ...
                    <Grid item xs={12} md={9}>
                        <Grid container spacing={3}>
                            <FormGrid item xs={12}>
                                <FormLabel sx={{ fontSize: '1.30rem' }} htmlFor="item-name">Item Name</FormLabel>
                                <OutlinedInput
                                    id="item-name"
                                    name="title"
                                    type="text"
                            ...
                            <FormGrid item xs={12} md={6}>
                                ...
                                <Select sx={{ fontSize: '1.30rem' }} id="item-type" name="item_type" value={formData.item_type} onChange={handleChange}>
                            ...
                            <FormGrid item xs={12}>
                                <FormLabel sx={{ fontSize: '1.30rem' }} htmlFor="item-description">Item Description</FormLabel>
                                <OutlinedInput
                                    id="item-description"
                                    name="description"
                                    type="text"
                            ...
                            <FormGrid item xs={12} md={6}>
                                <FormLabel sx={{ fontSize: '1.30rem' }} htmlFor="auction-type">Auction Strategy</FormLabel>
                                <Box sx={{ fontSize: '1.1rem' }}>{auctionDescriptions[formData.auction_strategy]}</Box>
                                <Select sx={{ fontSize: '1.30rem' }} id="auction-type" name="auction_strategy" value={formData.auction_strategy} onChange={handleChange}>
                            ...
                            <FormGrid item xs={12} md={4}>
                                <FormLabel sx={{ fontSize: '1.30rem'  }} htmlFor="start-price" required>
                                Starting Price
                                </FormLabel>
                                <OutlinedInput
                                id="start-price"
                            ...
                            <FormGrid item xs={12} md={6}>
                                <FormLabel sx={{ fontSize: '1.30rem' }} htmlFor="start-date">Auction Start Date</FormLabel>
                                <OutlinedInput
                                    id="start-date"
                                    name="start_at"
                                    type="datetime-local"
                            ...
                            <FormGrid item xs={12} md={6}>
                                <FormLabel sx={{ fontSize: '1.30rem' }} htmlFor="end-date">Auction End Date</FormLabel>
                                <OutlinedInput
                                    id="end-date"
                                    name="end_at"
                                    type="datetime-local"
                ...
                <Box sx={{ marginTop: 2, paddingRight:5, display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
                <Button
                    sx={{
                        fontSize: '1.30rem',
